...
- 複数サーバー構成への同時デプロイ
- 任意バージョンへのダウングレード
- MySQL/PostgreSQL の完全な自動バックアップ・復元
...
- MySQL/PostgreSQL は PHP だけで確実な完全バックアップを作る前提にしない。実行前に外部バックアップ済みの確認を必須にする。
...
1. 対応対象を SQLite のみとするか、MySQL/PostgreSQL も画面更新の対象にするか。
2. 利用中ホスティングで PHP の実行時間延長、`ZipArchive`、外部 HTTPS、ファイル rename が利用可能か。
3. 更新中の許容停止時間と、CLI を優先できる環境の割合。
4. GitHub Release を信頼基点とした SHA-256 検証で開始するか、署名付き manifest まで初回から導入するか。
5. システム管理者を「最初のユーザー」とする移行方針で問題ないか。
